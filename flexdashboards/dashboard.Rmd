...
```{r datasets}
videos <- read.csv("data/academatica_videos.csv")
metada <- read.csv("data/academatica_videos_metadata.csv")
...
metricas <- 
  ...
  summarise(total_views = sum(viewCount),
            ...
            totat_dislikes = sum(dislikeCount),
...
```{r}
like_rate = metricas$total_likes/(metricas$total_likes+metricas$totat_dislikes)
like_rate = round(like_rate*100,0)

gauge(like_rate, min = 0, max = 100, symbol = '%', gaugeSectors(success = c(80,100),
                                                                warning = c(40,79),
                                                                danger = c(0,39)))
```
...
```{r}
dislike_rate = metricas$totat_dislikes/(metricas$total_likes+metricas$totat_dislikes)
dislike_rate = round(dislike_rate*100,0)

gauge(dislike_rate, min = 0, max = 100, symbol = '%', gaugeSectors(success = c(0,39),
                                                                warning = c(40,79),
                                                                danger = c(80,100)))
```
...
```{r}
videos %>%
  mutate(year = year(ymd_hms(contentDetails.videoPublishedAt)), month = month(ymd_hms(contentDetails.videoPublishedAt)), year = as.factor(year)) %>%
  group_by(year, month) %>%
  summarise(upload_videos = n_distinct(contentDetails.videoId)) %>%
  ggplot(aes(x=month, y=upload_videos, fill=year))+ geom_col(position='dodge')
```
...
```{r}
docs <- Corpus(VectorSource(metada$title))
...
stats %>%
  ...
  filter(has_like) %>%
  left_join(metada, by = c("id"="video_id"))%>%
